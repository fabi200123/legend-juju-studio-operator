...
class LegendStudioCharm(base_operator.BaseFinosLegendCoreServiceCharm):
    ...
    def __init__(self, *args):
        ...
        self.framework.observe(
            self.on[SDLC_RELATION_NAME].relation_joined,
            self._on_sdlc_relation_joined)
        self.framework.observe(
            self.on[SDLC_RELATION_NAME].relation_changed,
            self._on_sdlc_relation_changed)
        self.framework.observe(
            self.on[SDLC_RELATION_NAME].relation_broken,
            self._on_sdlc_relation_broken)

        # Engine relation events:
        self.framework.observe(
            self.on[ENGINE_RELATION_NAME].relation_joined,
            self._on_engine_relation_joined)
        self.framework.observe(
            self.on[ENGINE_RELATION_NAME].relation_changed,
            self._on_engine_relation_changed)
        self.framework.observe(
            self.on[ENGINE_RELATION_NAME].relation_broken,
            self._on_engine_relation_broken)
    ...
    def _get_legend_gitlab_redirect_uris(self):
        ...
        redirect_uris = [
            STUDIO_GITLAB_REDIRECT_URI_FORMAT % {
                "base_url": base_url}]
    ...
    def _get_core_legend_service_configs(
        ...
        try:
            for service in service_urls:
                service_urls[service] = (
                    self._get_legend_service_url_from_relaton(service))
        except Exception:
            logger.error(
                "Exception occurred while getting Legend service URLs: %s",
        ...
        if missing:
            return model.WaitingStatus(
                "awaiting relations with following services: %s" % (
                    ", ".join(missing)))
        studio_ui_config = {
            "appName": "studio",
            "env": "test",
            "sdlc": {
                "url": service_urls["sdlc"]
            },
            "metadata": {
                "url": "__LEGEND_DEPOT_URL__/api"
            },
            "engine": {
                "url": service_urls["engine"]
            },
            "documentation": {
        ...
        return {
            STUDIO_UI_CONFIG_FILE_CONTAINER_LOCAL_PATH: (
                json.dumps(studio_ui_config, indent=4)),
            STUDIO_HTTP_CONFIG_FILE_CONTAINER_LOCAL_PATH: (
    ...
    def _on_engine_relation_broken(
            self, _: charm.RelationBrokenEvent) -> None:
        self._refresh_charm_status()
